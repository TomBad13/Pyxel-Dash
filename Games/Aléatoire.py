    ...
    def __init__(self):
        ...
        for i in range(64, self.lengh, 8):
            if PassTurn == True:
                ...
                RandomAppear = random.randint(1,11)
                if RandomAppear <= 3: #BLOC
                    self.bloc.append(Bloc(i,self.spawnpoint[1]))
                    if random.randint(1,6) == 1 and PastItemLog[-1] != "P"and PastItemLog[-1] != "V" and PastItemLog[-1] != "D" and PastItemLog[-2] != "P" and PastItemLog[-3] != "P" and PastItemLog[-2] != "V" and PastItemLog[-3] != "V" and PastItemLog[-2] != "D":
                        self.pique.append(Pique(i,self.spawnpoint[1]-16,True))
                        PastItemLog.append("PR")
                        for j in range(0, self.spawnpoint[1]-16, 8):
                            self.corde.append(Corde(i,j))
                    else:
                        PastItemLog.append("B")
                elif RandomAppear <= 7: #PIQUE
                    self.bloc.append(Bloc(i,self.spawnpoint[1]))
